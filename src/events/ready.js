...
const { logDeploy, reconstruirDoCanal } = require('../services/auditoria');
...
module.exports = {
  ...
  async execute(client) {
    ...
    // Se DB estiver vazio após sync, tenta reconstruir do canal de auditoria
    sincronizarCanal(client, { reconciliar: true })
      .then(async ({ novos, removidos }) => {
        console.log(`[sync/startup] novos=${novos} removidos=${removidos}`);
        await logDeploy(client, { novos, removidos });
        if (lerVeiculos().length === 0) {
          console.log('[startup] DB vazio após sync — tentando recovery do canal de auditoria...');
          await reconstruirDoCanal(client);
        }
      })
